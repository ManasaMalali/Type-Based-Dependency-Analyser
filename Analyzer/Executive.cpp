...
// add dep anal package and include that and display and strong components
...
void CodeAnalysisExecutive::processSourceCodeExec(bool showProc)
{
...
Files CodeAnalysis::CodeAnalysisExecutive::filesfromMap()
{
	FileMap m = getFileMap();
	Files v;
	Files tempFile;
...
void tempExec(CodeAnalysisExecutive &exec)
{
	std::ostringstream tOut("Type Based Dependency Analysis");
...
void ExecRequirements(CodeAnalysisExecutive &exec, std::string depPath, std::string strPath)
{
	TypeTable tb;											
	tb.GenerateTypeTable();
	Files filesDep = exec.filesfromMap();
	DependencyTable dp;
	NoSQLDB::NoSqlDb<std::string> db;
	Display disp;
	std::string resultString, resultString1;
	dp.DepAnal(filesDep, tb.returnTypeTable());
	db = dp.getDb();
	std::cout << std::endl;
	std::cout << "----------------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 9 - Automated unit test suite that demonstrates all the requirements of this project are met. " << std::endl;
	std::cout << "----------------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "--------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 8 - Processing command line arguments, with list of specified file patterns to match. " << std::endl;
	std::cout << "--------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "-------------------------------------------------------------------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 3 - Enforced Single Responsibity Principle- and the packages used are: Dependency Analysis, Display, Type Table, StrongComponents, TestExecutive " << std::endl;
	std::cout << "I have used my NoSqlDB from Project 1 [not professor's]" << std::endl;
	std::cout << "-------------------------------------------------------------------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "-------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 4 - TypeAnalysis package that identifies all of the types defined in a specified file" << std::endl;
	std::cout << "-------------------------------------------------------------------------------------------------" << std::endl;
	disp.TypeTableDisplay(tb.returnTypeTable());
	std::cout << "--------------------------------------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 5 - DependencyAnalysis package that identifies all of the dependencies between files in a specified file collection." << std::endl;
	std::cout << "--------------------------------------------------------------------------------------------------------------------------------" << std::endl;
	disp.DependencyDisplay(dp.getDb());
	std::cout << "--------------------------------------------------------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 6 - Find strong components in the dependency graph defined by the relationships evaluated in the previous requirement." << std::endl;
	std::cout << "--------------------------------------------------------------------------------------------------------------------------------" << std::endl;
	Strongcomponents scc;
	scc.ConstructGraph(dp);
	disp.StrongComponentDisplay(scc.returnMatrix());
	std::cout << "-------------------------------------------------------------------------------" << std::endl;
	std::cout << "REQUIREMENT 7 - Write the analysis results, in XML format, to a specified file." << std::endl;
	std::cout << "-------------------------------------------------------------------------------" << std::endl;
	std::cout << "\n -------------------------------XML to show the Dependency of Files----------------------------------- \n";
	resultString = db.toXml(depPath);
	std::cout << resultString << std::endl;
	std::cout << "\n  ------------------------------XML to show Strongly connected components---------------------------- \n";
	resultString1 = scc.sccToXml(strPath);
	std::cout << resultString1 << std::endl;
}
...
int main(int argc, char* argv[])
{
  ...
  try {
	...
	tempExec(exec);
	std::string str1 = "DependencyXml.xml";
	std::string str2 = "StrongComponentXml.xml";
	if (argc == 7)
	{
		str1 = FileSystem::Path::getFullFileSpec(argv[6]);
		if (!(FileSystem::Directory::exists(str1)))
		{
			std::cout << "Specify correct file path for xml file " << std::endl;
			return 0;
		}
		str1 += "\\DependencyXml.xml";
	}
	else if (argc == 8)
	{
		str1 = FileSystem::Path::getFullFileSpec(argv[6]);
		str2 = FileSystem::Path::getFullFileSpec(argv[7]);
		if (!(FileSystem::Directory::exists(str1) || FileSystem::Directory::exists(str2)))
		{
			std::cout << "Specify correct file paths for xml file " << std::endl;
			return 0;
		}
		str1 += "\\DependencyXml.xml";
		str2 += "\\StrongComponentXml.xml";
	}
	ExecRequirements(exec, str1, str2);	
  }
